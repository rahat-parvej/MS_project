...
def evalute_result(request):
    if request.method=='POST':
        ...
        score = model.evaluate(data_array, label_array)
        
        # Clean up - remove the uploaded zip file and the temporary directory
        shutil.rmtree(temp_dir)

        return render(request, 'evaluate/test_result.html', {'loss': score[0],'acc': score[1],'pre': score[2],'rec': score[3],'auc': score[4]})
